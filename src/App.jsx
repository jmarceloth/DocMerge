...
import { g3, combinePDFs, optimizePDF } from './lib/pdfUtils';
...
function App() {
  ...
  const [optimizeFilename, setOptimizeFilename] = useState("otimizado.pdf");
  const [optimizeQuality, setOptimizeQuality] = useState(0.80);
  const [isOptimizing, setIsOptimizing] = useState(false);
  const [optimizeProgress, setOptimizeProgress] = useState(0);
  const [originalSize, setOriginalSize] = useState(0);
  const [optimizedSize, setOptimizedSize] = useState(0);
  const optimizeInputRef = useRef(null);
  ...
  const handleOptimizeFileChange = (file) => {
  ...
  const handleOptimize = async () => {
    if (!optimizeFile) {
      alert("Selecione um arquivo PDF para otimizar");
      return;
    }
    ...
    setOptimizedSize(0);
    try {
      const blob = await optimizePDF(optimizeFile, optimizeQuality, (progress) => {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = optimizeFilename;
      a.click();
      URL.revokeObjectURL(url);
    } catch (error) {
      console.error(error);
      alert("Falha ao otimizar PDF");
    } finally {
